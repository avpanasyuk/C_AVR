...
namespace avp {
  namespace ADC_ { // we can not use name "ADC" because it is #defined as a name of register
    ...
    inline void SelectReference( enum Reference Ref ) {  avp::setbits(ADMUX, Ref, 2, REFS0); }
    ...
    inline void SelectChannel( uint8_t /* enum Channel */ Ch ) { avp::setbits(ADMUX, Ch, 4, MUX0); }
    ...
    inline void SetPrescaler() { avp::setbits(ADCSRA,avp::CeilLog2Ratio(F_CPU,200000UL),3,ADPS0); }


    // ******** ADCSRB *************
    enum AutoTriggerSource { FREE, AN_COMPARATOR, EXT_INT_0, TIMER0_COMPA, TIMER0_OVR,  TIMER1_COMPB, TIMER1_OVR, TIMER1_CAPT };
    inline void SetAutoTriggerSource( enum AutoTriggerSource Src ) { avp::setbits( ADCSRB, Src, 3, ADTS0); }
