...
namespace avp {
  ...
  protected:
    ...
    enum Status_ get_status() {
      while(TWCR & (1 << TWINT) == 0);
      return TWSR & (0xFF - 0x3);
    };
  public:
    void Init() {
      ...
      // set pins
      SCL::config(INPUT,HIGH); // pull up
      SDA::config(INPUT,HIGH); // pull up
      // setup
      TWCR = 1 << TWEN | 1 << TWEA; // enable module, no interrupts, send ACK on receive
    } // Init

    void send(uint8_t const *pb, uint16_t Size) { // Master transmitter
    ...
    void send(uint8_t b) { send(&b,1); }

    void receive(uint8_t *pb, uint16_t *pSize)  {
      TWCR |= 1 << TWSTA | 1 << TWINT;
      AVP_ASSERT(get_status() == START);
